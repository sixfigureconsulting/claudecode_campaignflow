...
// Slack webhooks are URLs so show as text, not password
const PLAIN_TEXT_SERVICES = new Set(["slack"]);
...
export function IntegrationConfigForm({
  ...
  const handleSave = async (service: ServiceKey) => {
    ...
    const res = await fetch(`/api/integrations/${projectId}`, {
      method: "POST",
    ...
    if (res.ok) {
      setFeedback((f) => ({ ...f, [service]: { type: "success", msg: "Key saved successfully" } }));
      setKeys((k) => ({ ...k, [service]: "" }));
      router.refresh();
    } else {
  ...
  const handleDelete = async (service: string) => {
    setDeleting(service);
    await fetch(`/api/integrations/${projectId}?service=${service}`, { method: "DELETE" });
    setDeleting(null);
    router.refresh();
  };
