...
export default async function ReportPage({
  ...
  const { clientId, projectId, reportId } = await params;
    ...
    .select(`
      *,
      ...
      projects!inner(
        ...
        clients!inner(id, name, user_id, industry, primary_offer, website)
      )
    `)
    .eq("id", reportId)
  ...
  // Subscription check for AI gating
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
        ...
        <span>/</span>
        <Link href={`/clients/${clientId}/projects/${projectId}`} className="hover:text-foreground">
          {project.name}
        </Link>
        <span>/</span>
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm flex items-center gap-1.5">
          ...
          {formatDate(report.report_date)} · {client.name} · {project.name}
        </p>
        {report.notes && (
